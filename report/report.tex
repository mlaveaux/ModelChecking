...
\begin{algorithmic}[1]
	\Procedure{NaiveEval}{$S, \to_S,f, vars,S_0$}    
...
\begin{algorithmic}[1]
	\Procedure{DiamondEval}{$S, \to_S,f, vars$}    
        	\State $R_1 \gets Eval(S,\to_S,f.sub_1,vars)$ 
        	\State $R_2 \gets \emptyset$ 
        	\For{\textbf{each} $(q,a,q\prime) \in \to_S$}
            	\If{$a = f.label$ \textbf{and} $q\prime \in R_1$}
...
\begin{algorithmic}[1]
	\Procedure{BoxEval}{$S, \to_S,f, vars$}    
        	\State $R_1 \gets Eval(S,\to_S,f.sub_1,vars)$ 
        	\State $R_2 \gets S$ 
        	\For{\textbf{each} $(q,a,q\prime) \in \to_S$}
            	\If{$a = f.label$ \textbf{and} $q\prime \not\in R_1$}
...
\begin{algorithmic}[1]
	\Procedure{ImprovedEval}{$S, \to_S,f, vars,S_0$}   
...
\begin{algorithmic}[1]
	\Procedure{InitVars}{$vars,f,S$}   
		...
		\ElsIf{$f.OP = MU$} 
			...
			\State $InitVars(vars,f.sub_1,S)$
		\Else
			\If{$f.OP = AND$ \textbf{or} $f.OP = OR$} 
				\State $InitVars(vars,f.sub_1,S)$
				\State $InitVars(vars,f.sub_2,S)$
			\ElsIf{$f.OP = BOX$ \textbf{or}  $f.OP = DIAMOND$} 
				\State $InitVars(vars,f.sub_1,S)$
			\EndIf
		\EndIf
	\EndProcedure
...
\begin{algorithmic}[1]
	\Procedure{Reset}{$S, f, vars, OriginalOP, SurroundingBinder$}    
		\If {$f.open = true$ \textbf{and} ($f.OP = MU$ \textbf{or} \textbf{$f.OP = NU$})}
			\If{$f.OP = OriginalOP$}
...
\begin{tikzpicture}
    ...
    \addplot+ [color=blue] coordinates {(2, 0.002) (3, 0.002) (4, 0.002) (5, 0.002) (6, 0.004) (7, 0.007) (8, 0.012) (9, 0.026) (10, 0.096)}; 
    
    \addplot+ [color=red] coordinates {(2, 0.002) (3, 0.002) (4, 0.002) (5, 0.003) (6, 0.004) (7, 0.007) (8, 0.014) (9, 0.024) (10, 0.087)}; 
  	\end{axis}
\end{tikzpicture}
...
3a: There is a play such that player II can win (holds, except for 350 robots).
...
3b: Player II wins the game (holds, except for 50, 200, 350 and 500 robots).
...
3c: Player II wins the game by being able to move the token to place (N, N) infinitely often (holds except for 200, 350, 
...
\begin{tikzpicture}
    ...
    \addplot+ [color=blue] coordinates {(50, 0.024) (100, 1.883) (150, 8.219) (200, 1.881) (250, 44.914) (300, 83.155) (350, 0.196) (400, 218.213) (450, 41.323)}; 
...
\begin{thebibliography}{9}
	...
	\bibitem{elpaper} E.A. Emerson and C.-L. Lei: Model Checking in the Propositional Mu-Calculus, TR-86-06, February 1986
\end{thebibliography}
